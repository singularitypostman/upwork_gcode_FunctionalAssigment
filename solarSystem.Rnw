%based on http://users.stat.umn.edu/~geyer/Sweave/foo.Rnw


\documentclass{article}
\title{SCC.212 Functional Assignment.  \newline based on http://users.stat.umn.edu/~geyer/Sweave/foo.Rnw}
\author{David Bowes}
\begin{document}
...
And now for the output from your R script:


<<echo=FALSE>>=
source(file="solarSystem.R")
@
